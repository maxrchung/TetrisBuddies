...
GameGrid::GameGrid(){
	...
	//numColors = 5; // Tetris Attack has 4 colors on easy, 5 colors on med/hard, and 6 in multi (grey blocks that become garbage)
...
std::array<int, 6> GameLogic::CreateTempRow(){
...
void GameGrid::InitialBoardPopulation(){
	...
	//	for (int j = 0; j < boardWidth; j++) {
	for (int rowNum = 0; i < 8; rowNum++) {
		for (int colNum = 0; colNum < 6; j++) {	
...
int GameLogic::SwapPieces(int row1Num, int col1Num, row2num, row2col){
